...
# Прямі заяви про первинний ринок: продавець або забудовник каже це словами.
_PRIMARY_EXPLICIT = re.compile(
    r"від\s+забудовник|перв(?:инн|ичн)\w+\s+ринок|здача\s+в\s+\d{4}|"
    r"введен\w*\s+в\s+експлуатац",
    re.I,
)
# Слабка ознака: сучасний будинок. Сама по собі не каже, хто продає — забудовник
# чи власник, який уже купив. Лишається в справі, бо на даних поводиться саме
# як новобудова: медіана $/м² у цій групі (1955 / 1866 / 1828 для 1-3 кімнат)
# вища за явну вторинку (1804 / 1534 / 1190) і не нижча за явну новобудову.
# Але поступається будь-якому конкретному факту — року будівництва чи прямій
# згадці вторинного ринку.
_PRIMARY = re.compile(
...
_SECONDARY = re.compile(
    ...
    r"житлов\w+\s+фонд\s+(?:від\s+)?(?:19|20[01])\d|"
    r"втор(?:инн|ичн)\w+\s+ринок|старий\s+фонд",
    re.I,
)
# Рік будівництва, витягнутий із тексту («житловий фонд від 2011 р.»). Це факт,
# а не формулювання, тому він сильніший за будь-який зі шаблонів вище.
# Рік здачі, названий прямо: «будинок зданий у 2023», «збудовано 2010».
#
# Формулювання «житловий фонд …» сюди навмисно НЕ входить, хоч спокуса є.
# OLX має категорію «житловий фонд ВІД 2021 р.», що означає «2021 і новіші»,
# тобто найновіші будинки. Прочитати цей рік як дату здачі — значить оголосити
# 1096 новобудов вторинкою. Цей випадок уже акуратно розібраний у `_SECONDARY`,
# де межа років свідомо обмежена 19xx і 200x–201x.
#
# Діапазон («будинок 2021-2025») означає чергу забудови: цікавить його кінець.
_BUILT_IN_TEXT = re.compile(
    r"(?:будинок|будинку|збудован\w*|здан\w*)\s*(?:у|в)?\s*"
    r"(19\d\d|20\d\d)(?:\s*[-–—]\s*(19\d\d|20\d\d))?",
    re.I,
)
# Скільки років будинок ще вважається таким, що формує первинний ринок.
NEW_BUILDING_YEARS = 2
...
def classify_market(*parts: str | None, built_year: int | None = None,
    ...
    """Первинний vs вторинний ринок, за силою доказу.

    Зміна проти попередньої версії рівно одна, і вона виправляє конкретну
    ваду: слабка ознака «ЖК» стояла першою й перемагала пряму згадку
    вторинного ринку. Оголошення «квартира в ЖК, житловий фонд від 2011 р.»
    ішло як новобудова, хоч будинку півтора десятка років.

    Порядок:

    1. Пряма згадка вторинного ринку — «вторинний ринок», «хрущовка»,
       «житловий фонд 2011». Це заява, а не здогад.
    2. Пряма згадка первинного — «від забудовника», «здача в 2027».
    3. Слабка ознака — «новобудова», «ЖК». Лишається в справі, бо на даних
       ця група поводиться саме як первинний ринок: медіана за м²
       1955 / 1866 / 1828 для 1–3 кімнат проти 1804 / 1534 / 1190 у явної
       вторинки.
    4. Рік будівництва — останнім, як і було. Спокуса поставити його вище
       велика: це ж факт. Але перевірка показала, що об'єкти, які він
       перекидав би у вторинку (будинки 2019–2023 років), коштують як
       новобудови — 1162 проти 1209 у новобудов і 1583 у вторинки. Великі
       комплекси продаються забудовником роками після здачі, тож сам по собі
       рік здачі про продавця нічого не каже.
    """
    text = " ".join(p for p in parts if p)

    if _SECONDARY.search(text):
        return MarketType.SECONDARY
    if _PRIMARY_EXPLICIT.search(text):
        return MarketType.PRIMARY
    if _PRIMARY.search(text):
        return MarketType.PRIMARY

    year = built_year
    if year is None and (m := _BUILT_IN_TEXT.search(text)):
        year = int(m.group(2) or m.group(1))
    if year:
        return (MarketType.PRIMARY if year >= datetime.now().year - NEW_BUILDING_YEARS
                else MarketType.SECONDARY)
