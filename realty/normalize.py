...
def classify_condition(*parts: str | None, market: "MarketType | None" = None) -> Condition:
    """Стан житла за описом/тегами. Заперечення мають пріоритет.

    `market` дозволяє дотягнути окремий випадок: оголошення про квартиру в
    ЖК, який ще не зданий. Про ремонт там не пишуть, бо квартири поки немає,
    але фактично вона переходить до покупця без оздоблення.
    """
    ...
    if market is MarketType.PRIMARY and _UNBUILT.search(text):
        return Condition.NEEDS_REPAIR
    return Condition.UNKNOWN
